...
const Products = ({ value }) => {
  const [btnactv, setBtnActv] = useState({
    btn1: true,
    btn2: false,
    btn3: false,
  });
        ...
        <div className="text-center">
          ...
          <div className="text-start buttonBox ps-md-5">
            <button
              className={
                btnactv.btn1
                  ? "btn productsBoxBtn px-3 btnActive"
                  : "btn productsBoxBtn px-3"
              }
              name="btn1"
              onClick={handleBtn}
            >
          ...
          <div className="border px-3 pt-4">
            <Row xs={1} md={3} lg={4} className="g-4">
              {items?.map((item) => (
                <Product value={value} key={item.product_code} item={item} />
              ))}
            </Row>
          </div>
        </div>
